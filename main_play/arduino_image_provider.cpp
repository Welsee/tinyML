...
#if !(defined OV2640_MINI_2MP_PLUS)                                         //우리 카메라 모델
...
#define MAX_JPEG_BYTES 4096                                                 //JPEG파일 최대 크기 설정 64x64 인듯?
// The pin connected to the Arducam Chip Select
#define CS 7

// Camera library instance
ArduCAM myCAM(OV2640, CS);
// Temporary buffer for holding JPEG data from camera                        //카메라 버퍼에 이미지를 임시저장하기 위해 초기화
uint8_t jpeg_buffer[MAX_JPEG_BYTES] = {0};
// Length of the JPEG data currently in the buffer
uint32_t jpeg_length = 0;                                   //초기 길이는 0으로 설정

// Get the camera module ready
TfLiteStatus InitCamera(tflite::ErrorReporter* error_reporter) {          //tflite파일에 에러 메세지 출력
  TF_LITE_REPORT_ERROR(error_reporter, "Attempting to start Arducam");    //error-reporter에 에러만 있는게 아니라 출력 문구들 선언된듯
  // Enable the Wire library
  Wire.begin();
  // Configure the CS pin
  pinMode(CS, OUTPUT);                                                    //cs핀은 7번
  ...
  if (test != 0x55) {
    TF_LITE_REPORT_ERROR(error_reporter, "Can't communicate with Arducam");     //카메라 상호작용 확인
    delay(1000);
    return kTfLiteError;
  }
  // Use JPEG capture mode, since it allows us to specify
  // a resolution smaller than the full sensor frame
  myCAM.set_format(JPEG);                                                   //카메라가 JPEG로 받아들이게 설정
  myCAM.InitCAM();                                                         //카메라 시작
  // Specify the smallest possible resolution
  myCAM.OV2640_set_JPEG_size(OV2640_160x120);                                //사이즈 설정
  delay(100);
  return kTfLiteOk;
}
...
TfLiteStatus ReadData(tflite::ErrorReporter* error_reporter) {                  //카메라 버퍼에 저장된 이미지를 읽는 함수
  ...
  if (jpeg_length == 0) {
    TF_LITE_REPORT_ERROR(error_reporter, "No data in Arducam FIFO buffer");         //데이터가 0일땐 에러
    return kTfLiteError;
  }
  myCAM.CS_LOW();
...
// Decode the JPEG image, crop it, and convert it to greyscale                          //greyscale로 변환, 색깔은 판단 불가
